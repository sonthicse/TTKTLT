...
void CLI::showAdd() {
  recalc();
  clrInner();
  drawTitle("THÊM MỚI HỒ SƠ SÁCH (M1)");
  drawGuide(
      "Huong dan: [↑]/[↓] chuyen o | [Enter]: o tiep | [Esc]: quay lai MENU");
  ...
  vector<string> lbls = {"Ma the loai:",
                         ...
                         "Gia sach:"};
  const int fcnt = static_cast<int>(lbls.size());
  vector<string> vals(fcnt, "");
  vector<int> tops(fcnt);
  ...
  auto redraw = [&]() {
    for (int i = 0; i < fcnt; ++i) {
      bool sel = (i == cf);
      drawLblBox(fl, tops[i], bw, lbls[i], vals[i], sel);
    }
  };
  ...
  while (!done) {
    ...
    if (iw < 1)
      iw = 1;
    int tlen = static_cast<int>(vals[cf].size());
    if (tlen > iw)
      tlen = iw;
    ...
    if (c == 27) {
      ...
      if (static_cast<int>(vals[cf].size()) < maxLen) {
        vals[cf].push_back(static_cast<char>(c));
      }
    }
  }
}
...
void CLI::showList() {
  ...
  while (inScr) {
    if (redrawT) {
      ...
      int pgX = max(2, cw - static_cast<int>(pgInfo.size()) - 3);
